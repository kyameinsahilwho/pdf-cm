...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    tmpInPath = path.join(tmpDir, `ppt_pdf_in_${randomId}${ext}`);
    tmpOutPath = path.join(tmpDir, `ppt_pdf_out_${randomId}.pdf`);

    await fs.writeFile(tmpInPath, buffer);

    const scriptPath = path.join(process.cwd(), 'api', 'convert-office-pdf.py');

    await runPythonScript(scriptPath, ['ppt-to-pdf', tmpInPath, tmpOutPath], { timeout: 120000 });
    ...
    console.error('[ppt-to-pdf API] Error:', err);
    return NextResponse.json({ error: err?.message || 'Failed converting PowerPoint to PDF' }, { status: 500 });
  } finally {
